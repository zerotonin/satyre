...
sys.path.append('/home/iaji/Documents/codes/good_codes')
...
maxTrial = 100
allFoodFound_orl_df= []
#allMovementTypes_orl_df = []
#allAreaCovered_orl_df = []
allPositionCoords_orl_df = []
#allPositionCoords_dataframe = []
allStepSize_orl_df = []
allThrustVelocities_orl_df = []
allSlipVelocities_orl_df = []
allYawVelocities_orl_df = []
allYawAngles_orl_df = []
allPsiAngles_orl_df = []
allTailPositions_orl_df = []
allHeadPositions_orl_df = []
for i in [0,3]:
    ...
    ws.simulateMultiple()   
    allFoodFound_orl_df.append(ws.foodFound_total_allTrials)
    allPositionCoords_orl_df.append(ws.positionCoords_allTrials)
#    allPositionCoords_dataframe.append(ws.positionCoords_dataframes)
    #    allMovementTypes_orl_df.append(ws.TransVsRot_proportion_allTrials)
    #    allAreaCovered_orl_df.append(ws.areaCovered_allTrials)
    allStepSize_orl_df.append(ws.stepSize_allTrials)
    allThrustVelocities_orl_df.append(ws.thrustVelocities_allTrials)
    allSlipVelocities_orl_df.append(ws.slipVelocities_allTrials)
    allYawVelocities_orl_df.append(ws.yawVelocities_allTrials)
    allYawAngles_orl_df.append(ws.yawAngles_allTrials)
    allPsiAngles_orl_df.append(ws.psiAngles_allTrials)
    allTailPositions_orl_df.append(ws.tailCoords_allTrials)
    allHeadPositions_orl_df.append(ws.headCoords_allTrials)
...
time_start = time.clock()
i=0
from walkSimOO_updated_darkFly_v13 import walkSim_darkFly
ws=walkSim_darkFly(rawDataList[i], rawDataList[i+1], rawDataList[i+2], nrTrial = 3,
            border = 10000,maxSteps = 10000,totalFood = 1000,foodMode='random')    
ws.simulate()  
#plt.plot(ws.foodPos[:,0], ws.foodPos[:,1], 'd', markersize=5)
#plt.title('Foods dispersed randomly', fontsize=36)
ws.saveAllData()
plt.figure()
ws.plotTrajectories()
...
#cProfile.run('ws.simulate()')


from walkSimOO_updated_LevyRandom_expBased_darkCond_multiTrials import walkSim
ws=walkSim(cauchyAlpha =1, nrTrial = 3,
                border = 10000.0, mode='uniform', maxSteps = 10000,totalFood = 1000,foodMode='clustered')
ws.simulate()  
ws.saveAllData()
plt.figure()
ws.plotTrajectories()
...
import cProfile
cProfile.run('ws.simulate_stopAtFoodBorder()')
time_elapsed2 = (time.clock() - time_start) 

a = np.load('/media/gwdg-backup/BackUp/Irene/simulationResults/df_clusteredFood_darkCond/simulationResults/allHeadPositionsX_df.npy')
